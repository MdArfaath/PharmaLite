...
| Testing | **vitest (wired)** — unit tests for schemas/pure logic; `npm test`. -library/react, playwright still later |
...
| Expiry alerts | ✅ **Complete & verified** — v_expiring_soon list grouped Expired/Today/Soon; shows name, batch, date, remaining days, stock; nearest-first; links to detail; auto-refresh via ["medicines"] key nesting; buckets + window exclusion tested on Postgres |
| Sales history | ✅ **Complete & verified** — reverse-chron list w/ infinite scroll, bill-no.+medicine-name search, date-range filter; detail w/ line items+grand total; void (void_sale RPC restores stock); auto-refresh via ["sales"] key; tested on Postgres |
| Settings | ✅ **Complete & verified** — editable shop name, phone, low-stock threshold, expiry window, currency; Zod-validated; RLS-scoped shops UPDATE w/ settings-jsonb merge; loading/error/success states; unit tests (vitest); persistence + tenant isolation tested on Postgres |
| PWA polish | ⏳ Pending |
...
| 2026-07-31 | **Expiry Alerts module built & verified.** Added `features/alerts/expiryQueries.ts` (`useExpiring` reading `v_expiring_soon`; `ExpiringRow` type), `features/alerts/expiryStatus.ts` (pure `classifyExpiry` → expired/today/soon + remaining-days label), `components/alerts/ExpiryCard.tsx` (status tag, name, batch, expiry date, remaining days, stock via `StockBadge`; links to detail), and screen `alerts/expiry/{page,ExpiryList}.tsx` (grouped Expired/Expiring-today/Expiring-soon sections, nearest-first, states). **No DB/auth/inventory/add-stock/sell/low-stock/middleware/schema/app-shell changes** — reused `StockBadge`, `formatQty`, `States`, `date-fns`. Decisions: (1) **Query key `["medicines","expiring"]`** so existing `["medicines"]` invalidations auto-refresh it (same pattern as low-stock). (2) **Bucketing done client-side** via `classifyExpiry` from `expiry_date` (view supplies `is_expired` + window, but "today" vs "soon" needs day-precision); grouped into sections for a clear visual distinction. (3) Sorted by `expiry_date` ascending (nearest/expired first). Verified: build+typecheck+lint green; on real Postgres confirmed `v_expiring_soon` returns expired (is_expired=t) + today rows, excludes a +200-day medicine (outside 30-day window), ordered nearest-first; and `classifyExpiry` boundaries (−5/−1/0/1/7/30) produce correct buckets + singular/plural labels. Dev DB restored after tests. |
| 2026-07-31 | **Sales History module built & verified.** Added `features/sales/billNumber.ts` (derives a stable `#XXXXXX` bill no. from the sale UUID — no invoice column in v1), `features/sales/queries.ts` (`useSalesList` infinite query with date-range + text search resolving medicine-name hits via `sale_items` and bill-no./id-prefix hits; `useSaleDetail` header+items; `useVoidSale` via `void_sale` RPC), components `sales/{SaleRow,SalesFilterBar}`, list screen `sales/{page,SalesList}` (infinite scroll via IntersectionObserver), detail screen `sales/[saleId]/{page,SaleDetailScreen}` (meta, line items, grand total, Void action + ConfirmDialog). **No DB/auth/inventory/add-stock/sell/alerts/middleware/schema/app-shell changes** — reused `SearchInput`, `ConfirmDialog`, `States`, `formatMoney`, `date-fns`. Decisions: (1) **Bill number derived from UUID** (schema has no invoice no.; invoices are out of scope) — search matches this + medicine name. (2) **"Customer" mapped to the existing `note` field** ("Customer / note") — no customer column exists; shown only when present. (3) **List keyed `["sales","list",filters]`** so the Sell mutation's `["sales"]` invalidation auto-refreshes after a new sale. (4) Void uses the append-only `void_sale` RPC (kept + struck-through, stock restored); voided sales stay visible in history. (5) Infinite scroll at 20/page. Verified: build+typecheck+lint green; on real Postgres confirmed reverse-chron list, medicine-name + id-prefix search, date-range filter, detail line items w/ snapshots + grand total, void restores stock (Paracetamol +2, sale→voided), and **cross-tenant fully blocked** (shop B sees 0 of A's sales; void rejected). Dev DB restored after tests. |
| 2026-07-31 | **Settings module completed & verified.** Added `features/settings/schema.ts` (Zod: name, optional phone, low-stock threshold 0–100000, expiry window 1–365, currency enum), `features/settings/queries.ts` (`useShop`, `useUpdateShop` RLS-scoped shops UPDATE, and pure exported `toShopUpdate` that merges the settings jsonb preserving unmanaged keys), `app/(app)/settings/SettingsForm.tsx` (RHF+Zod, dirty-aware Save, toast success/error), and rewrote `app/(app)/settings/page.tsx` to render the editable form (kept account link + sign-out). **First tests in the repo:** added `vitest@2.1.9` (dev), `vitest.config.ts` (node env, `@/` alias, `*.test.ts` only), `test`/`test:watch` scripts, and 15 unit tests across `schema.test.ts` + `queries.test.ts` — all passing. **No DB/auth/routing/middleware/app-shell/inventory/sales/alerts/schema changes.** Decisions: (1) **Settings persisted via `shops.settings` jsonb merge** (no schema change; unmanaged/future keys preserved — unit-tested). (2) **Added currency as an editable preference** (already in settings jsonb; enum INR/USD/EUR/GBP/AED). (3) **Save disabled until dirty**, re-baselined after save. (4) Mutation invalidates `["shop"]`, `["dashboard-stats"]`, `["medicines"]` so threshold/window changes immediately affect the alert views. Verified: build+typecheck+lint+tests green; on real Postgres confirmed the shops UPDATE persists name/phone/merged-settings, the changed threshold flows into `v_low_stock` (effective_threshold=7), and **cross-tenant shop UPDATE blocked by RLS** (0 rows; shop A unchanged). Dev DB restored after tests. **All v1 modules now complete.** |
